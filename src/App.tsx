...
const App: React.FC = () => {
  ...
  const handleLogin = async (email: string, password?: string) => {
    ...
    if (foundUser) {
      ...
      if (foundUser.role === 'ADMIN') {
        ...
        } catch (e) { console.warn("Erro ao persistir admin."); }
      }
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-900 text-white gap-4">
      ...
      <p className="font-black tracking-widest text-xs uppercase animate-pulse">Iniciando Portal Niklaus...</p>
    </div>
  );
  ...
  // Se NÃO for admin e NÃO tiver CNPJs, bloqueia. Se FOR admin, passa direto.
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 text-center">
        <div className="max-w-md bg-white p-12 rounded-[3rem] shadow-xl border border-slate-100 animate-fade-in">
          ...
          <p className="text-slate-500 font-medium leading-relaxed mb-6">Olá, <strong>{user.name}</strong>! Seu cadastro foi recebido. Um administrador entrará em contato em breve.</p>
          <button onClick={handleLogout} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black hover:bg-black transition-all">Sair</button>
        </div>
      </div>
    );
  }

  // Se for admin e não houver nada no banco, força a página de Backoffice
  if (user.role === 'ADMIN' && (allowedCnpjs.length === 0 || selectedCnpj === null)) {
    return (
      <div className="flex min-h-screen bg-slate-50 text-slate-900">
        <Sidebar currentPage="backoffice" onPageChange={setCurrentPage} userRole={user.role} />
        <div className="flex-1 flex flex-col h-screen overflow-hidden">
          <Navbar user={user} selectedCnpj={{ name: 'Modo Admin', id: 'none', number: '000', distributor: 'Niklaus', trayGroupId: 'group_basic' }} onSwitchCnpj={() => {}} onLogout={handleLogout} cartCount={0} />
