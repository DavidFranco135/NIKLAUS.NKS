...
const History: React.FC<HistoryProps> = ({ user, orders }) => {
  const [filterCnpj, setFilterCnpj] = useState<string>('all');

  // Filtra apenas pedidos que pertencem aos CNPJs do usuário logado
  const userCnpjIds = user.cnpjs.map(c => c.id);
                ...
                {filteredOrders.map(order => (
                  <tr key={order.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-8 py-6">
                       <p className="text-sm font-bold text-gray-900 leading-tight">{user.cnpjs.find(c => c.id === order.cnpjId)?.name || 'Unidade B2B'}</p>
